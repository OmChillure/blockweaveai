  ...
  const fetchAllProxies = async () => {
    ...
    try {
      ...
      const formattedEntries = allEntries.map((entry) => ({
        ...
        price: entry.account.price.toNumber() / web3.LAMPORTS_PER_SOL,
        purchased: entry.account.purchased,
        buyer: entry.account.buyer,
      }));

      setEntries(formattedEntries);
    } catch (error) {
      console.error('Error fetching proxies:', error);
      setError('Failed to fetch proxy entries. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
