...
import { RefreshCcw, SendIcon, StopCircleIcon, TrashIcon, InfoIcon } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-[2fr,3fr] gap-6">
          <div className="relative aspect-square lg:aspect-auto rounded-xl overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
              <Button variant="secondary" className="mb-2">{type.toUpperCase()}</Button>
            </div>
          </div>
          <div className="flex flex-col gap-6 p-6">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold mb-2">{dataset.title ?? "Global Climate Change Dataset"}</h1>
              <div className="flex items-center text-gray-400 text-sm">
                <InfoIcon size={16} className="mr-2" />
                <ScrollArea className="w-full h-6">
                  <p className="whitespace-nowrap">{dataset.ipfsHash ?? "By Climate Research Institute"}</p>
                </ScrollArea>
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Button 
                ...
                onClick={() => downloadFile(
                  `https://${process.env.PINATA_GATEWAY_D}/ipfs/${dataset.ipfsHash}`,
                  `${dataset.title}.py`
                )}
              >
                    ...
                    <div className="space-y-4">
                      {messages.map(message => (
                        <div key={message.id} className={`flex flex-col ${message.role === 'user' ? "items-end" : "items-start"}`}>
                          <div className={`p-3 rounded-lg max-w-[80%] ${message.role === 'user' ? "bg-blue-600" : "bg-gray-800"}`}>
                            {message.content}
                          </div>
